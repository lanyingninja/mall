...
if($("#pass").val()==$("#pass1").val()){
	...
	$("#tijiao").click(
	function(){
		 if($("#user").val().length>3&&$("#user").val().length<16&&reg.test($("#pass").val())&&$("#pass").val()==$("#pass1").val()&&reg1.test($("#phone").val())&&reg11.test($("#email").val())){
			$("form").submit();
				}else{
...
<form action="UserServlets?opr=reg" method="post">
	<table width="511px" cellspacing="4px" cellpadding="1px" style="margin-left:100px;" >
		...
		<td colspan="2"><input type="text" placeholder="仅限大陆手机用户"  style="display:block; width:250px; height:35px" id="phone" name="phone"/></td>
		    <td id="v4">&nbsp;</td>
		  </tr>
		  <tr>
		    <td height="85"  style="font-size:13px; font-weight:bold; color:white" >邮箱号码</td>
		<td colspan="2"><input type="text" placeholder="仅限大陆邮箱"  style="display:block; width:250px; height:35px" id="email" name="email"/></td>
		    <td id="v5">&nbsp;</td>
		  </tr>
		  <tr>
